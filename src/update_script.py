...
import traceback  # ✅ ADDED (no behavior change)
...
# ----------------- PATCH 1: SAFE SPARK INIT -----------------
 
# ----------------- PATCH 2: THREAD SAFETY -----------------
def _safe_future_result(fut, label="thread"):
    try:
        return fut.result()
    except Exception:
...
# ----------------- PATCH 3: FORCE EARLY EXECUTION -----------------
def _validate_df(df, name="df"):
    try:
        print(f"[DEBUG] validating {name}")
        df.limit(1).count()  # triggers plan
    except Exception:
        print(f"🔥 DF validation failed: {name}")
        traceback.print_exc()
        raise
 
# ----------------- PATCH 4: WRITE GUARD -----------------
def _guard_write(write_callable, label):
    try:
        print(f"[DEBUG] write -> {label}")
        write_callable()
    except Exception:
        print(f"🔥 WRITE FAILED: {label}")
        traceback.print_exc()
        raise
...
try:
    ...
    try:
        ...
        print(f"\n🔥 [FINALLY] Unexpected error during cleanup: {_finally_err}")
        traceback.print_exc()
        raise
